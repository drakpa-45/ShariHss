...
import bt.gov.ditt.paroHss.common.SmsSender;
import bt.gov.ditt.paroHss.dao.IDaoCitizen;
import bt.gov.ditt.paroHss.dao.IDaoCommon;
import bt.gov.ditt.paroHss.dao.IsavePrivateLandDao;
...
public class ServiceCitizen implements IServiceCitizen {
    ...
    public OnlineTimberDTO saveOnlineTimberApplication(OnlineTimberDTO timberDto, HttpServletRequest request) {
        ...
        try {
            ...
            System.out.println("----- phone number ----------"+timberDto.getPhone_Number() + "================= before message sent ===============");
            SmsSender.smsSender(timberDto.getPhone_Number(), "", null, smsContent, "Application Registered Success");
            System.out.println("----- smsContent ----------"+smsContent + "================= after message sent ===============");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseMessage getTimberReplacementRequest(String appNo) {
        ...
        if(replacementCount<1 || appStatus.equals(7)){
            if(appStatus.equals(18) || appStatus.equals(7)){
                ...
                responseMessage.setText("Replacement of RTP can be available after Processing of sawing permit.!");
            }
        }else{
            responseMessage.setStatus(0);
