...
import bt.gov.ditt.paroHss.common.utils.CommonNotificationUtils;
import bt.gov.ditt.paroHss.dao.IDaoDealing;
import bt.gov.ditt.paroHss.dao.IDaoRange;
    ...
    public WorkFlowDto upadteClaimedApp(WorkFlowDto workFlow, ListDto listDto, HttpServletRequest request) {
        ...
        for(int m=0; m<userBean.getRoles().length;m++){
            for(int i=0; i<userBean.getRoles()[m].getServices().length;i++){
                bt.gov.ditt.paroHss.dto.Service svc = userBean.getCurrentRole().getServices()[i];
